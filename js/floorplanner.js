...
var CameraButtons = function(floorplannercreater) {

  var orbitControls = floorplannercreater.three.controls;
  var three = floorplannercreater.three;
...
var ContextMenu = function(floorplannercreater) {
...
var ModalEffects = function(floorplannercreater) {

  var scope = this;
  var floorplannercreater = floorplannercreater;
  var itemsLoading = 0;
  ...
  function init() {
    floorplannercreater.model.scene.itemLoadingCallbacks.add(function() {
      itemsLoading += 1;
      update();
    });

     floorplannercreater.model.scene.itemLoadedCallbacks.add(function() {
      itemsLoading -= 1;
      update();
    });   
...
var SideMenu = function(floorplannercreater, floorplanControls, modalEffects) {
  var floorplannercreater = floorplannercreater;
  ...
  function init() {
    ...
    floorplannercreater.three.updateWindowSize();
    handleWindowResize();
  ...
  function tabClicked(tab) {
    return function() {
      ...
      floorplannercreater.three.stopSpin();
  ...
  function setCurrentState(newState) {
    ...
    // set item unselected
    floorplannercreater.three.getController().setSelectedObject(null);
    ...
    if (currentState == scope.states.FLOORPLAN) {
      floorplannercreater.model.floorplan.update();
    }
  ...
  function initItems() {
    $("#add-items").find(".add-item").mousedown(function(e) {
      ...
      floorplannercreater.model.scene.addItem(itemType, modelUrl, metadata);
      setCurrentState(scope.states.DEFAULT);
    });
  }
...
var TextureSelector = function (floorplannercreater, sideMenu) {
...
var ViewerFloorplanner = function(floorplannercreater) {
  ...
  this.floorplanner = floorplannercreater.floorplanner;
...
var mainControls = function(floorplannercreater) {
  ...
  function loadDesign() {
    ...
    reader.onload = function(event) {
        ...
        floorplannercreater.model.loadSerialized(data);
    }
    reader.readAsText(files[0]);
  }

  function saveDesign() {
    var data = floorplannercreater.model.exportSerialized();
    var a = window.document.createElement('a');
    var blob = new Blob([data], {type : 'text'});
    a.href = window.URL.createObjectURL(blob);
    a.download = 'design.floorplannercreater';
...
$(document).ready(function() {
  ...
  var floorplannercreater = new Blueprint3d(opts);

  var modalEffects = new ModalEffects(floorplannercreater);
  var viewerFloorplanner = new ViewerFloorplanner(floorplannercreater);
  var contextMenu = new ContextMenu(floorplannercreater);
  var sideMenu = new SideMenu(floorplannercreater, viewerFloorplanner, modalEffects);
  var textureSelector = new TextureSelector(floorplannercreater, sideMenu);        
  var cameraButtons = new CameraButtons(floorplannercreater);
  mainControls(floorplannercreater);
